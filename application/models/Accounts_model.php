...
class Accounts_model extends CI_Model
{
	...
	*/
    ...
    {           
        $sql = "SELECT 	dv.debit_voucher_id,
						dv.voucher_no, 
						dv.current_date,
						dv.pay_status,
						dv.pay_name,
				...
				WHERE 	dv.my_id = {$my_id} 
					ORDER BY dv.debit_voucher_id ASC";
    ...
    {           
        $sql = "SELECT 	dv.debit_voucher_id,
				...
				inner join tbl_general_voucher as gv 
					on dv.voucher_no = gv.voucher_no
				WHERE 	dv.my_id = {$my_id} 
					and	dv.debit_voucher_id = {$debit_voucher_id}
					ORDER BY dv.debit_voucher_id ASC";
    ...
    {           
        $sql = "SELECT 	bk.devoucher_bkdn_id,
				...
				inner join tbl_devoucher_bkdn as bk 
					on dv.debit_voucher_id = bk.debit_voucher_id
				WHERE 	dv.my_id = {$my_id} 
					and	dv.debit_voucher_id = {$debit_voucher_id}
    ...
    {           
        $sql = "SELECT 	cr.credit_voucher_id, 
						cr.voucher_no, 
						cr.current_date,
						cr.pay_to,
						cr.purpose,
				...
				WHERE 	cr.my_id = {$my_id} 
					ORDER BY cr.credit_voucher_id ASC";
    ...
    {           
        $sql = "SELECT 	cr.credit_voucher_id, 
				...
				inner join tbl_general_voucher as gv 
					on cr.voucher_no = gv.voucher_no
				WHERE 	cr.my_id = {$my_id} 
					and	cr.credit_voucher_id = {$debit_voucher_id}
					ORDER BY cr.credit_voucher_id ASC";
    ...
    {           
        $sql = "SELECT 	bk.crvoucher_bkdn_id,
						bk.particulars,
						bk.account_code,
						bk.table_name,
						bk.table_id,
				...
				inner join tbl_crvoucher_bkdn as bk 
					on cr.credit_voucher_id = bk.credit_voucher_id
				WHERE 	cr.my_id = {$my_id} 
					and	cr.credit_voucher_id = {$debit_voucher_id}
					ORDER BY bk.crvoucher_bkdn_id ASC";
	...
	public function get_data($a){
		$sql = "SELECT * FROM tbl_fcoa_master WHERE master_code like '%$a%'";
		$query = $this->db->query($sql);
		if($query){
			foreach ($query->result() as $row){
				$sql2="SELECT * FROM tbl_fcoa WHERE fcoa_master_id=$row->fcoa_master_id and fcoa_code like '%$a%'";
				$query2 = $this->db->query($sql2);
				if($query2){
					foreach ($query2->result() as $row2){
						$sql3="SELECT * FROM tbl_fcoa_bkdn WHERE fcoa_id=$row2->fcoa_id and bkdn_code like '%$a%'";
						$query3 = $this->db->query($sql3);
						if($query3){
							foreach ($query3->result() as $row3){
								$sql4="SELECT * FROM tbl_fcoa_bkdn_sub WHERE fcoa_bkdn_id=$row2->fcoa_bkdn_id and sub_code like '%$a%'";
