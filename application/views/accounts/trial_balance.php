				...
				if (sizeof($get_master_head) > 0) {
					...
					if (!empty($mster_debit_value) or !empty($mster_credit_value)) {
						...
						foreach ($get_master_head as $fcoa_head_info) {
							...
							if (!empty($fcoa_head_info["dr"])) {
								$fcoa_head_info_dr = "[DR " . $fcoa_head_info["dr"] . "]";
							} else {
								$fcoa_head_info_dr = '';
							}
							if (!empty($fcoa_head_info["cr"])) {
								$fcoa_head_info_cr = "[CR " . $fcoa_head_info["cr"] . "]";
							} else {
								$fcoa_head_info_cr = '';
							}
									...
									for ($fcoa_head_inc = 0; $fcoa_head_inc < sizeof($fcoa_head_ref_exp) - 1; $fcoa_head_inc++) {
										...
										if (sizeof($get_fcoa_bkdn) > 0) {
											...
											foreach ($get_fcoa_bkdn as $fcoa_bkdn_info) {
												...
												if (!empty($fcoa_bkdn_info["dr"])) {
													$fcoa_bkdn_info_dr = "[DR " . $fcoa_bkdn_info["dr"] . "]";
												} else {
													$fcoa_bkdn_info_dr = '';
												}
												if (!empty($fcoa_bkdn_info["cr"])) {
													$fcoa_bkdn_info_cr = "[CR " . $fcoa_bkdn_info["cr"] . "]";
												} else {
													$fcoa_bkdn_info_cr = '';
												}
														...
														for ($sub_fcoa_head_inc = 0; $sub_fcoa_head_inc < sizeof($sub_fcoa_head_ref_exp) - 1; $sub_fcoa_head_inc++) {
															...
															if (sizeof($get_sub_fcoa_bkdn) > 0) {
																...
																foreach ($get_sub_fcoa_bkdn as $sub_fcoa_bkdn_info) {
																	...
																	if (!empty($sub_fcoa_bkdn_info["dr"])) {
																		$sub_fcoa_bkdn_info_dr = "[DR " . $sub_fcoa_bkdn_info["dr"] . "]";
																	} else {
																		$sub_fcoa_bkdn_info_dr = '';
																	}

																	if (!empty($sub_fcoa_bkdn_info["cr"])) {
																		$sub_fcoa_bkdn_info_cr = "[CR " . $sub_fcoa_bkdn_info["cr"] . "]";
																	} else {
																		$sub_fcoa_bkdn_info_cr = '';
																	}
